"""CerebrasProvider — implementación de ILLMProvider usando el SDK de Cerebras.

JSON mode via response_format={"type": "json_object"}.
Free tier: 1M tokens/día — 10× más que Groq.
Incluye exponential backoff para rate limit.
"""
...
class CerebrasProvider(ILLMProvider):
    """Implementación Cerebras. Sustituible por cualquier ILLMProvider."""
        ...
        for attempt in range(self._max_retries):
            try:
                ...
                if (is_rate_limit or is_transient) and attempt < self._max_retries - 1:
                    ...
                    print(f"  [CerebrasProvider] {label} — esperando {wait}s (intento {attempt + 1}/{self._max_retries})...")
                    time.sleep(wait)
                elif is_rate_limit:
                    match = re.search(r"try again in ([\d.]+)s", err_str, re.IGNORECASE)
                    retry_hint = f"Reintenta en ~{match.group(1)}s" if match else "Reintenta en unos minutos"
                    raise RuntimeError(
                        f"RATE_LIMIT: Cerebras rate limit agotado tras {self._max_retries} intentos. "
